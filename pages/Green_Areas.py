...
# green area map
espacios_verdes = pd.read_csv('espacios_verdes.csv', sep=';', encoding='utf8')
...
espacios_verdes = espacios_verdes.rename(columns={espacios_verdes.columns[6]:'Numero Zonas Socializacion Canina', espacios_verdes.columns[7]:'Numero Elementos Fitness'})

huerto_distrito = pd.DataFrame(espacios_verdes.groupby('DM')['Superficie Huerto Urbano'].sum().reset_index())
caninos_distrito = pd.DataFrame(espacios_verdes.groupby('DM')['Numero Zonas Socializacion Canina'].sum().reset_index())
fitness_distrito = pd.DataFrame(espacios_verdes.groupby('DM')['Numero Elementos Fitness'].sum().reset_index())


todos = pd.merge(huerto_distrito, caninos_distrito, on='DM')
todos = pd.merge(todos, fitness_distrito, on ='DM')
todos = todos.rename(columns={'Superficie Huerto Urbano': 'Urban garden', 'Numero Zonas Socializacion Canina': 'Canine areas', 'Numero Elementos Fitness': 'Fitness areas'})
...
contenedores_aceite = pd.read_csv('contenedores_aceite.csv', sep=';', encoding='utf8')
nuevos_nombres = {'objectid':'ID', 'Tipus / Tipo': 'Tipo', 'Centre / Centro': 'Centro', 'Adreça / Direccion' : 'Direccion', 'Districte / Distrito': 'Distrito', 'Barri / Barrio': 'Barrio', 'Oli / Aceite': 'Aceite', 'Estat / Estado': 'Estado'}
contenedores_aceite = contenedores_aceite.rename(columns=nuevos_nombres)

contenedores_pilas = pd.read_csv('contenedores_pilas.csv', sep=';', encoding='utf8')
nuevos_nombres = {'objectid':'ID', 'Tipus / Tipo': 'Tipo', 'Centre / Centro': 'Centro', 'Adreça / Direccion' : 'Direccion', 'Districte / Distrito': 'Distrito', 'Barri / Barrio': 'Barrio', 'Piles / Pilas': 'Pilas', 'Observació / Observación': 'Observacion', 'Codi / Código': 'Codigo'}
contenedores_pilas = contenedores_pilas.rename(columns=nuevos_nombres)

contenedores_ropa = pd.read_csv('contenedores_ropa.csv', sep=';', encoding='utf8' )
...
todos = todos.rename(columns={todos.columns[0]: 'Oil', todos.columns[1]: 'Batteries', todos.columns[2]: 'Clothes'})
